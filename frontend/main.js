...
async function loadReport(silent = false) {
  ...
  try {
    ...
    try {
      ...
      reportData = parseHtmlReport(rawText);
    }

    if (!Array.isArray(reportData)) {
      console.error("Formato inesperado:", reportData);
...
function parseHtmlReport(html) {
  ...
  rows.forEach((row, index) => {
    ...
    result.push({
      ...
      total: Number(cells[1].textContent.trim().replace(",", ".")),
      revenue: Number(cells[2].textContent.trim().replace(",", ".")),
    });
  });
...
function renderReportTable(data, tableBody) {
  ...
  for (const item of data) {
    ...
    row.innerHTML = `
      <td>${item.type}</td>
      <td>${item.total}</td>
      <td>R$ ${Number(item.revenue).toFixed(2).replace(".", ",")}</td>
    `;
...
function updateDashboardStats(data) {
  ...
  document.getElementById("stat-carros").textContent = totalCar;
  document.getElementById("stat-motos").textContent = totalMoto;
  document.getElementById("stat-caminhoes").textContent = totalTruck;
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (reportButton) reportButton.addEventListener("click", () => loadReport());
